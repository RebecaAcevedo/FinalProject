...
#names <- Re_finalcounts$gene_id
#names <- make.names(nams, unique=TRUE)
#row.names(Re_final.count.table) <- names
...
sample.group <- gsub("\\.[0-9]+", "", names(recount.count.table))
View(sample.group)
#sample.group <-names(final.count.table)
Final_info <- data.frame(row.names=names(recount.count.table), condition=sample.group)
...
RecountFinal.ds <- DESeqDataSetFromMatrix (countData = recount.count.table, colData = Final_info, design = ~condition) #design refers to a column(s) in colData (here, ~condition indicates the "condition" column) 
...
RecountFinal.ds <- estimateSizeFactors (RecountFinal.ds)
# if you check colData() again, you can see it now contains the sizeFactors
colData (RecountFinal.ds)
...
RecountFinal_DESeq.rlog <- rlog(RecountFinal.ds, blind = TRUE) # use blind = FALSE if large differences exist in a large proportion of genes.
RecountFinal_rlog.counts <- assay(RecountFinal_DESeq.rlog) # assay() function gets the read counts (integer) or normalized values (decimal), depending on the argument value
head(RecountFinal_rlog.counts) # check and compare to head(assay(DESeq.ds)) 
...
head(RecountFinal_rlog.counts, n=10)
cor(RecountFinal_rlog.counts[,1], RecountFinal_rlog.counts[,10]) #default method is Pearson
cor(RecountFinal_rlog.counts[,1], RecountFinal_rlog.counts[,10], method = "spearman")

cor(RecountFinal_rlog.counts, method = "spearman")


# clustering
# distance measure is d = 1 - r
distance.rlog <- as.dist (1 - cor(RecountFinal_rlog.counts)) 
...
library (ggplot2)
plotPCA (RecountFinal_DESeq.rlog) # Apply to DESeqDataSet; it may not be used with non-normalized read counts
...
RecountFinal.ds <-  DESeq(RecountFinal.ds)
...
RecountDE.results <- results (RecountFinal.ds, contrast = c('condition','Tumor_sample','Non.tumor_sample'), alpha = 0.05) 
...
summary (RecountDE.results) 

#check the content of the DGE.results object (similar to a data frame)
head (RecountDE.results, 10)
table (RecountDE.results$padj < 0.05) #to build a contingency table of the counts at each factor level (here, TRUE/FALSE).
table (RecountDE.results$log2FoldChange > 1)
row.names (subset(RecountDE.results, padj < 0.05))

# shrink log2FoldChanges (for low expression) and generate an updated result table
Recountres.shrunk <- lfcShrink(RecountFinal.ds, contrast = c('condition','Tumor_sample','Non.tumor_sample'), res=RecountDE.results, type = 'normal')
head(Recountres.shrunk, 10)# compare the result tables: head(res.shrunk) vs. head(DE.results)
class(Recountres.shrunk) #check the object type by class() function
...
plotMA (RecountDE.results, alpha = 0.05, ylim = c(-5,5)) # genes that pass padj < 0.05 are colored in blue
plotMA (Recountres.shrunk, alpha = 0.05, ylim = c(-5,5))
...
EnhancedVolcano(Recountres.shrunk, lab = rownames(Recountres.shrunk), x ='log2FoldChange', y ='padj')
EnhancedVolcano(Recountres.shrunk, lab = rownames(Recountres.shrunk), x ='log2FoldChange', y ='padj', pCutoff = 10e-50, FCcutoff = 2) # the default pCutoff is 10e-6; default FCcutoff is abs(log2FC) > 1) 
...
EnhancedVolcano(Recountres.shrunk, lab = rownames(Recountres.shrunk), x ='log2FoldChange', y ='padj', pCutoff = 10e-3, FCcutoff = 2, selectLab = 'ENSG00000132698') 

# Histograms based on padj
# the frequency (y-axis) of certain values (x-axis) present in a data set
hist (RecountDE.results$padj)
hist (RecountDE.results$padj, 
      col='grey', breaks=50, 
...
RecountDEgenes <- row.names(subset(RecountDE.results, padj < 0.05))

# extract the normalized read counts (from Sec. 2) for DE genes into a matrix
#mat_DEgenes <- rlog.counts[DEgenes, ]
Recountmat_DEgenes <- RecountFinal_rlog.counts[RecountDEgenes, c('Tumor_sample.1', 'Non.tumor_sample.1')]

# plot the normalized read counts of DE genes 
aheatmap (Recountmat_DEgenes, Rowv = NA , Colv = NA)
